...
bool ImGui::button_tab(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int page, int in, bool border, ImColor clr) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (page == in)
	{
		PushStyleColor(ImGuiCol_Text, clr);
		RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.5, 0.5), &bb);
		PopStyleColor();

		window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y) - ImVec2(0, 1), bb.Max - ImVec2(0, 1), false);
	}
	else
	{
		PushStyleColor(ImGuiCol_Text, ImVec4(.65f, .65f, .65f, 1));
		RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.5, 0.5), &bb);
		PopStyleColor();
	}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	ImRect grab_bb;
	if (is_horizontal)
		grab_bb = ImRect(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + grab_padding), ImVec2(grab_pos, frame_bb.Max.y - grab_padding));
	else
...
#include "../../src/Utilities/item_definitions.hpp"
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	auto color = ImVec4(game_data::menu_color[0], game_data::menu_color[1], game_data::menu_color[2], game_data::menu_color[3]);
	...
	else if (selected) PushStyleColor(ImGuiCol_Text, color);

	RenderTextClipped(bb.Min + ImVec2(4, 4), bb_with_spacing.Max + ImVec2(4, 4), label, NULL, &label_size, ImVec2(0.0f, 0.0f));
	if (flags & ImGuiSelectableFlags_Disabled || selected) PopStyleColor();
